...
#инициализируем библиотеку cv_bridge и создаем объект с нужным нам названием (в нашем случае bridge).Через него будем производить конвертацию сообщений ROS в изображения openCv и наоборот
bridge = CvBridge()
#цвет шрифта
...
#с красным цветом немного иначе, т.к в цветовом пространстве hsv ось H закольцована, а значит красными будут как величины близкие к 0 , так и близкие к 180, поэтому распознавать будем в два прохода и результат комбинировать
...
#объединение двух изображений сверху. Если точка белая хотя бы на одном из изображений, то она будет белой и на конечном
