С распознаванием цветов как в Python, так и в целом мы столкнулись впервые, поэтому было решено сначала поизучать теорию и поиграться с распознаванием при помощи вебки. Стоит отметить, что мы решили сделать 2 программы (1 распознает цвета, 2 qr-коды)
Тесты с вебкамерой помогли подобрать цветовые диапозоны и отладить сам алгоритм распознавания цвета, после чего были предприняты попытки написания кода под Клевер:
```python
# -*- coding: utf-8 -*-
